module Sig
  class CursoLato < Base
    ...
    def self.find_by_id_curso(id_curso)
      json_request("#{LATO_SENSU_CURSO_LATO_ID_CURSO_URL}/#{id_curso}")
    end
    def self.find_by_id_modalidade_educacao(id_modalidade_educacao)
      json_request("#{LATO_SENSU_CURSO_LATO_ID_MODALIDADE_EDUCACAO_URL}/#{id_modalidade_educacao}")
    end
    def self.all
      json_request("#{LATO_SENSU_CURSO_LATO_GET_ALL_URL}")
    end
  end
end